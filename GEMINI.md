...
El proyecto se estructura por dominios (DDD-lite) con una jerarquía aplanada y soporte para **Nested Routing**:
- **Estructura Aplanada**: Los dominios base (ej: `core`, `catalog`) contienen submódulos hermanos directamente para evitar profundidad excesiva (ej: `core/staff`, `core/org_units`).
- **Nested Routing**: Cada módulo raíz (ej: `core/routers.py`) actúa como un agregador unificado, permitiendo que el router global sea limpio y escalable.
- **Componentes por Módulo**:
    - `routers.py`: API Endpoints (Agregador o Específicos).
