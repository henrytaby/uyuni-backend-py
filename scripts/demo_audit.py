...
def run_demo():
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            print(
                f"{'ID':<5} | {'Action':<10} | {'Entity':<10} | {'User':<10} | {'Changes (Preview)'}"
            )
            print("-" * 80)
            for log in logs:
                changes_str = json.dumps(log.changes) if log.changes else ""
                changes_preview = (
                    (changes_str[:40] + "..") if len(changes_str) > 40 else changes_str
                )
                print(
                    f"{log.id:<5} | {log.action:<10} | {log.entity_type:<10} | {log.username or 'N/A':<10} | {changes_preview}"
                )

                if log.action == "UPDATE" and log.entity_type == "Task":
                    print(
                        f"   >>> Full Changes for Task Update: {json.dumps(log.changes, indent=2)}"
                    )
